...
# -*- coding: utf-8 -*-
"""
水位/雨量传感器数据模拟脚本 —— 100 传感器版本
=========================================
用途：模拟乡村暴雨夜场景下，水位传感器向后端定时上报读数，
     从而驱动后端规则引擎"系统自动触发预警"，便于联调与演示。

本版本面向 **100 传感器** 规模（系统默认部署规模）。
（1000 传感器压测请使用 sim_water_level_1000.py，处理逻辑完全一致）

两种模式：
  1) 单设备模式（--devices 1，默认）：直观演示单个设备的水位曲线变化。
  2) 多设备压测模式（--devices N）：模拟 N 个传感器并发周期上报，
     用于评估后端在 100 传感器规模下的 QPS、成功率与延迟表现。
...
def main():
    ...
    parser.add_argument("--scenario", default="surge",
                        ...
                        help="水位场景：flat/rise/surge/wave/mixed")
    parser.add_argument("--interval", type=float, default=2.0, help="上报间隔(秒)")
    parser.add_argument("--devices", type=int, default=1, help="模拟设备数，>1 进入压测模式")
